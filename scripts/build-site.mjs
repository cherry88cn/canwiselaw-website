...
const willEstateIntakeUrl = 'https://canwise-law-will-estate-intake.yiyao2025.chatgpt.site/client/will-estate-intake';
const zhFontUrl = '/assets/fonts/SourceHanSansCN-VF.otf.woff2';
const zhFontFaceCss = `@font-face{font-family:"Source Han Sans CN";src:url("${zhFontUrl}") format("woff2");font-style:normal;font-weight:250 900;font-display:swap}`;
const zhFontStack = `"Source Han Sans CN","Noto Sans CJK SC","Microsoft YaHei",sans-serif`;
...
const about = `<section class="page-hero"><div class="eyebrow">Our Philosophy</div><h1>A Bilingual Canadian Law Firm</h1><p>CanWise Law serves clients in English and Chinese in immigration, business and commercial, and family law matters.</p></section><section><div class="eyebrow">Our Firm</div><h2>Our Team</h2><div class="cards two"><article><h3>Hui Ding</h3><p><strong>Principal Lawyer and Founder</strong></p><p>Hui specializes in Canadian immigration and commercial law. She provides representation in Federal Court and strategic legal counsel to commercial clients, drawing on her diverse legal background.</p></article><article><h3>Asher Shu Tan</h3><p>Asher brings over a decade of experience in international student recruitment and immigration law. He has guided international students through the immigration journey and previously worked as a business lawyer and immigration consultant.</p></article></div></section><section class="soft"><div class="eyebrow">Legal Fees</div><h2>Transparent Fee Arrangements</h2><div class="cards three"><article><h3>Legal Consultation</h3><p class="price">CAD $350 per hour</p></article><article><h3>Flat Fees</h3><p>Available for selected services when scope and deliverables can be clearly defined in advance. HST, government fees, court fees and third-party disbursements are additional unless expressly stated.</p></article><article><h3>Hourly & Contingency Fees</h3><p>Hourly services are normally billed at CAD $350 per hour. Contingency arrangements are considered only for legally eligible matters after individual assessment and a compliant written agreement.</p></article></div></section>${cta('Begin Your Legal Journey','Connect with CanWise Law for a transparent consultation and structured guidance.')}`;
...
const contact = `<section class="page-hero"><div class="eyebrow">Direct Access</div><h1>Bilingual Consultation Inquiry</h1><p>Our team offers measured, culturally fluent advice. Contact us to outline your needs and we will respond regarding next steps.</p></section><section><div class="cards three"><article><h3>Office & Location</h3><p>2 Bloor Street E., Suite 3500<br>Toronto, Ontario M4W 1A8</p></article><article><h3>Contact</h3><p><a href="tel:+16476915569">647-691-5569</a><br><a href="mailto:admin@canwiselaw.com">admin@canwiselaw.com</a></p></article><article><h3>Office Hours</h3><p>Monday–Friday<br>9:00 AM–5:00 PM<br>Saturday and Sunday by appointment.</p></article></div></section>${cta('Schedule a Consultation','Consultations are available in English and Mandarin. The consultation fee is CAD $350 per hour.')}`;
...
const tagAliases = {
  'study permit':'学签', 'study permits':'学签', 'student permit':'学签', '学习许可':'学签',
  'work permit':'工签', 'work permits':'工签', '工作许可':'工签',
  'pgwp':'毕业工签', 'post-graduation work permit':'毕业工签',
  'judicial review':'司法复议', 'federal court':'联邦法院',
  'rcmp criminal record check':'RCMP无犯罪记录', 'fingerprinting':'录指纹', 'fingerprints':'录指纹',
  'express entry':'联邦快速移民', 'ee':'联邦快速移民', 'ircc':'加拿大移民局'
};
const normalizeTag = value => {
  const clean=value.trim().replace(/^#+/,'').trim();
  return tagAliases[clean.toLowerCase()] || clean;
};
const articleTags = article => [...new Set((article.tags || '').split(',').map(normalizeTag).filter(Boolean))];
const tagLink = tag => `<a href="/zh/blog/?tag=${encodeURIComponent(tag)}">#${tag}</a>`;
const tagList = article => `<div class="article-tags">${articleTags(article).map(tagLink).join('')}</div>`;
const articleCard = (article, href, readLabel) => `<article data-tags="${articleTags(article).join('|')}"><time>${article.date}</time><h3><a href="${href}">${article.title}</a></h3>${tagList(article)}<p>${articleBody(article).split(/\n{2,}/)[0].slice(0,150)}…</p><a href="${href}">${readLabel} →</a></article>`;
const allSiteTags = () => {
  const counts=new Map();
  for (const article of articles) for (const tag of articleTags(article)) counts.set(tag,(counts.get(tag)||0)+1);
  return [...counts].sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0],'zh-CN')).map(([tag])=>tag);
};
const articleSidebar = () => `<aside class="article-sidebar"><section><h2>免责声明/Disclaimer</h2><p>本网站文章仅供一般信息参考，不构成针对任何具体情况的法律意见。</p><p lang="en">The information on this website is provided for general informational purposes only and does not constitute legal advice for any specific matter.</p></section><section><h2>文章分类</h2><div class="sidebar-tags">${allSiteTags().map(tagLink).join('')}</div></section></aside>`;
...
function embedded(slug, lang='en') {
  ...
  if (lang === 'zh') {
    const embeddedZhTypography = `<style>${zhFontFaceCss}body,body *{font-family:${zhFontStack}!important}</style>`;
    srcdoc = srcdoc.includes('</head>')
      ? srcdoc.replace('</head>',`${embeddedZhTypography}</head>`)
      : `${embeddedZhTypography}${srcdoc}`;
  }
...
const zhPages = {
  ...
  about:`${zhHero('关于我们','一家提供中英双语服务的加拿大律师事务所','CanWise Law 为客户提供移民法、商业与公司法及家庭法服务。')}${zhCards([['Hui Ding','创办人及主任律师。主要从事加拿大移民法和商业法，并代理联邦法院案件，为商业客户提供策略性法律建议。'],['Kiki Zhang','国际教育及客户服务协调员。协助家庭进行中小学规划、学校申请，以及学生在加拿大就读期间的持续支持。'],['Jane Wang','法律支持助理。协助档案准备、文件整理、客户沟通及其他律师助理和行政工作。']])}<section class="soft"><h2>透明的收费安排</h2><div class="cards three"><article><h3>法律咨询</h3><p class="price">每小时 CAD $350</p></article><article><h3>固定费用</h3><p>部分范围清晰的服务可采用固定费用；HST、政府费、法院费及第三方支出另计。</p></article><article><h3>按小时及风险代理</h3><p>一般按每小时 CAD $350 计费。风险代理仅在法律允许且个案评估后以合规书面协议确定。</p></article></div></section>${zhCta()}`,
  ...
  'legal-consultation':`${zhHero('法律咨询','与律师讨论您的事务','就移民法、商业法和家庭法问题获得针对具体资料的初步法律建议。')}${zhCards([['咨询费','每小时 CAD $350。标准咨询为 60 分钟。'],['咨询前准备','简要整理您的情况和问题，并准备重要通知、协议、决定及往来文件。'],['会议安排','选择工作日时间，以中文或英文讨论。确认邮件会列明会议方式和准备说明。']])}<section class="soft"><h2>请注意</h2><p>除非 CanWise Law 后续书面同意接受委托，单次咨询不会自动建立持续的律师—客户关系。意见以咨询时提供的事实和文件为基础。</p></section>${zhCta('预约您的法律咨询')}`,
...
zhPages['business-commercial-fees']=zhFeeTable('商业与公司法服务费',[
 ['咨询与公司设立', [['法律咨询','$350 / 小时','商业结构、合同、风险及策略'],['安省公司设立','从 $1,500 起','标准注册、组织及电子公司记录册；政府费另计'],['联邦公司设立','从 $2,000 起','包括标准组织；省外注册费另计'],['专业公司','从 $2,500 起','监管批准及政府费另计']]],
...
zhPages['family-law-fees']=zhFeeTable('家庭法服务费',[
 ['咨询与意见', [['法律咨询、文件审阅或书面意见','$350 / 小时','法律意见、方案及下一步规划'],['独立法律意见（ILA）','从 $1,000 起','审阅家庭协议并提供意见；不含起草和谈判']]],
 ...
 ['谈判、争议与法院事项', [['谈判及往来函件','$350 / 小时','可能需要预付委托金'],['子女或配偶抚养费分析','个别报价','财务披露及计算；专家费另计'],['争议离婚或家庭法院申请','个别报价','评估后确认初始委托金和阶段预算'],['紧急动议','个别报价','视紧急程度、证据、听证要求及档期而定']]]
]);
...
const zhCss = `${zhFontFaceCss}html[lang="zh-CN"] body,html[lang="zh-CN"] body *{font-family:${zhFontStack}!important}.language-link{font-weight:700;color:var(--blue)!important}.fee-table{margin-top:30px;border-top:1px solid #dce2e7}.fee-row{display:grid;grid-template-columns:minmax(220px,1.1fr) minmax(130px,.45fr) minmax(260px,1.35fr);gap:22px;padding:18px 0;border-bottom:1px solid #dce2e7;align-items:start}.fee-row b{color:var(--blue)}.fee-row span{color:#566575}@media(max-width:720px){.fee-row{grid-template-columns:1fr;gap:5px}}`;
...
for (const article of articles) {
  ...
  const body=`<div class="article-layout"><article class="article"><a class="back-link" href="/zh/blog/">← 返回文章列表</a><div class="eyebrow">${article.section==='guides'?'实用百科':'政策与案例'}</div><h1>${article.title}</h1><time>${article.date}</time>${tagList(article)}<div class="article-body">${markdown(articleBody(article))}</div></article>${articleSidebar()}</div><section class="cta"><h2>咨询加拿大移民法律问题</h2><p>预约咨询，获取针对您具体情况的法律意见。</p>${button('预约法律咨询')}</section>`;
...
const zhContact=readPage('contact','../../')
  ...
  .replace('Schedule a Consultation','预约法律咨询').replace('Consultations are available in English and Mandarin. The consultation fee is CAD $350 per hour.','提供中文及英文咨询，咨询费为每小时 CAD $350。')
...
const willEstateIntakeFolder = path.join(repo,'client','will-estate-intake');
fs.mkdirSync(willEstateIntakeFolder,{recursive:true});
fs.writeFileSync(path.join(willEstateIntakeFolder,'index.html'),`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow,noarchive"><title>Will &amp; Estate Planning Questionnaire | CanWise Law</title><style>html,body{width:100%;height:100%;margin:0;overflow:hidden;background:#f7f6f1}.intake-frame{display:block;width:100%;height:100%;border:0;background:#f7f6f1}</style></head><body><iframe class="intake-frame" title="CanWise Law Will and Estate Planning Questionnaire" src="${willEstateIntakeUrl}"></iframe><noscript>This questionnaire requires JavaScript. Please contact CanWise Law for assistance.</noscript></body></html>`);
...
fs.writeFileSync(path.join(repo,'sitemap.xml'),`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${['',...routes,...articles.map(a=>`blog/${a.slug}`),'zh',...zhRoutes.filter(x=>x!=='home').map(x=>`zh/${x}`),'zh/blog','zh/contact'].map(r=>`<url><loc>${siteUrl}/${r}${r?'/':''}</loc></url>`).join('')}</urlset>`);
